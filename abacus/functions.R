...
## Anova, variando apenas tamanho amostral
sim.anova <- function(mu1, st.dev, sample.size, nrep, ...){
    dots <- list(...)
    ## f1 <- function(mu1, st.dev, sample.size, ...)
    ##     wp.anova(sd1=st.dev, N=sample.size, mu1=mu1, ...)
    ## raw <- replicate(nrep, mapply(f1, mu1, st.dev, sample.size, MoreArgs = dots),
    ##                  simplify=TRUE)
    raw <- matrix(nrow=8, ncol=nrep)
    for(i in 1:nrep)
        raw[,i] <- wp.anova(sd1=st.dev, N=sample.size, mu1=mu1)
    results <- c(sum(raw[6,])/nrep, # Prop rightful conclusions NHT
                 sum(raw[7,])/nrep, # Prop rightfull conclusions IT by the criterium 1:  deltaAIC = 0 for the right model
                 sum(raw[8,])/nrep, # Prop rightfull conclusions IT by the criterium 2: deltaAIC > 2 for all wrong models
                 sum(raw[6,]!=raw[7,])/nrep,# Prop mismatches in conclusions NHT x IT criterium 1
